// Attach authored model answers to the archived General Essay PYQs
// (data/pyq/essays-archive.json — essays from years whose full mains
// papers are not ingested, 2008–2024, APPSC + TGPSC).
//
// Reads:
//   data/pyq/essays-archive.json                  — question source (no, year, paper_label, text)
//   data/pyq/answers/essays-archive-answers.json  — { "Q7": "<markdown answer>", ... }
//
...
async function main() {
  const questions = JSON.parse(readFileSync(rel("data/pyq/essays-archive.json"), "utf8"));
  const answers = JSON.parse(
    readFileSync(rel("data/pyq/answers/essays-archive-answers.json"), "utf8")
  );
