// Attach authored model answers to the archived Paper-V PYQs
// (data/pyq/paper5-archive.json — Paper-V PYQs from years whose full mains
// papers are not ingested: 2008/2011/2012/2016/2017 APPSC + TGPSC 2016/2024).
//
// Reads:
//   data/pyq/paper5-archive.json                  — question source (no, year, paper_label, text)
//   data/pyq/answers/paper5-archive-answers.json  — { "Q7": "<markdown answer>", ... }
//
...
async function main() {
  const questions = JSON.parse(readFileSync(rel("data/pyq/paper5-archive.json"), "utf8"));
  const answers = JSON.parse(
    readFileSync(rel("data/pyq/answers/paper5-archive-answers.json"), "utf8")
  );
