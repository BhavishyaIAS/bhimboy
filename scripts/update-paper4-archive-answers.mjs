// Attach authored model answers to the archived Paper-IV PYQs
// (data/pyq/paper4-archive.json — Paper-IV PYQs from years whose full mains
// papers are not ingested: 2008/2011/2012/2016/2017 APPSC + TGPSC 2016/2024).
//
// Reads:
//   data/pyq/paper4-archive.json                  — question source (no, year, paper_label, text)
//   data/pyq/answers/paper4-archive-answers.json  — { "Q7": "<markdown answer>", ... }
//
...
async function main() {
  const questions = JSON.parse(readFileSync(rel("data/pyq/paper4-archive.json"), "utf8"));
  const answers = JSON.parse(
    readFileSync(rel("data/pyq/answers/paper4-archive-answers.json"), "utf8")
  );
  ...
  for (const q of questions) {
    ...
    if (!rows || rows.length === 0) {
      notFound++;
      console.error(`? ${qkey}: no matching row (${(q.year ?? 0)}, ${q.paper_label})`);
      continue;
    }
    for (const r of rows) {
