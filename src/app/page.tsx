...
export default async function LandingPage() {
  ...
  return (
    <main className="relative flex flex-1 flex-col overflow-hidden bg-[#0d0b0c] text-neutral-100">
      <OceanScene />

      <header className="relative z-10">
        <div className="mx-auto flex h-16 w-full max-w-5xl items-center justify-between px-4">
          <span className="font-display text-xl font-semibold italic tracking-tight text-neutral-100">
            Bhimboy
          </span>
      ...
      <section className="relative z-10 mx-auto flex min-h-[78vh] w-full max-w-3xl flex-col items-center justify-center px-4 pb-40 pt-10 text-center">
        <div className="animate-rise-in">
          <Lotus breathing className="mx-auto h-11 w-20 fill-neutral-200/90" />
          <p className="mt-6 font-display text-sm tracking-[0.3em] text-[#e0555b]">
            శాంతిః శాంతిః శాంతిః
          </p>
          <p className="mt-1 text-xs italic text-neutral-300/60">
            “Peace, peace, peace”
          </p>
          <h1 className="mx-auto mt-8 max-w-2xl font-display text-[2.5rem] font-medium leading-[1.14] tracking-tight text-neutral-50 sm:text-6xl">
            Still water.
            <span className="mt-1 block italic text-[#e0555b]">
              Deep preparation.
            </span>
          </h1>
          <p className="mx-auto mt-6 max-w-xl text-lg leading-8 text-neutral-300/85">
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Button
              asChild
              size="lg"
              className="rounded-full bg-primary px-8 text-base text-primary-foreground shadow-[0_10px_44px_-8px_rgba(216,31,38,0.45)] hover:bg-primary/90"
            >
            ...
            {!user && (
              <Button
                ...
                size="lg"
                variant="outline"
                className="rounded-full border-neutral-300/30 bg-white/5 px-8 text-base text-neutral-100 backdrop-blur hover:bg-white/10 hover:text-white"
              >
                <Link href="/login">I already study here</Link>
              </Button>
            )}
          </div>
          <p className="mt-8 text-xs tracking-wide text-neutral-400/80">
      ...
      {/* The three worlds */}
      <section className="relative z-10 border-t border-white/10 bg-gradient-to-b from-[#0a0809] to-[#120e0f] px-4 py-20">
        <div className="mx-auto max-w-5xl">
          <h2 className="text-center font-display text-2xl font-medium tracking-tight text-neutral-100 sm:text-3xl">
          ...
          <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-3">
            {worlds.map((wld, i) => (
              <div
                key={wld.name}
                className="group relative rounded-2xl border border-white/10 bg-white/[0.04] p-7 backdrop-blur transition-all duration-700 hover:border-[#d81f26]/40 hover:bg-white/[0.07]"
              >
                <span className="font-display text-xs uppercase tracking-[0.3em] text-[#e0555b]/70">
                  {`${i + 1} · ${wld.stage}`}
                </span>
                <div className="mt-4 flex h-11 w-11 items-center justify-center rounded-full bg-primary/15 text-[#e0555b]">
      ...
      {/* Closing */}
      <section className="relative z-10 bg-[#120e0f] px-4 pb-40 pt-16 text-center">
        <blockquote className="mx-auto max-w-xl font-display text-xl italic leading-9 text-[#e0555b] sm:text-2xl">
        ...
        <div className="mt-10">
          <Button
            asChild
            size="lg"
            className="rounded-full bg-primary px-10 text-primary-foreground hover:bg-primary/90"
          >
            <Link href={user ? "/app" : "/signup"}>
        ...
        <p className="mt-14 text-xs tracking-wide text-neutral-500">
